...
class Milestone2AcceptanceCriteriaTest {
     ...
     */
    @Test
    fun `AC-11-1 buying a good deducts gold and adds to inventory`() =
        runTest {
     ...
     */
    @Test
    fun `AC-11-2 selling a good adds gold and removes from inventory`() =
        runTest {
     ...
     */
    @Test
    fun `AC-11-3 profitable trade route earns gold`() =
        runTest {
     ...
     */
    @Test
    fun `AC-11-4 completedTradesCount increments on each successful trade`() =
        runTest {
            val before = gameRepository.observePlayerState().first()
     ...
     */
    @Test
    fun `AC-11-5 companion recruitment blocked until 3 trades completed`() =
        runTest {
     ...
     */
    @Test
    fun `AC-13-1 travel appends an arrival event to the event log`() =
        runTest {
     ...
     */
    @Test
    fun `AC-13-2 arrival event log meta includes destination townId`() =
        runTest {
            stepTrackerService.recordSensorDelta(count = 1000, source = StepSource.Simulation)
     ...
     */
    @Test
    fun `AC-13-3 event log persists after travel (survives restart simulation)`() =
        runTest {
            stepTrackerService.recordSensorDelta(count = 1000, source = StepSource.Simulation)
            val roads = gameRepository.observeRoadsFromCurrentTown().first()
     ...
     */
    @Test
    fun `AC-13-4 failed travel does not log an arrival event`() =
        runTest {
