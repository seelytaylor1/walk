...
class Milestone1AcceptanceCriteriaTest {
    ...
    @Test
    fun `AC-8-1 bankedSteps updates in DB after recording steps`() =
        runTest {
    ...
    @Test
    fun `AC-8-2 bankedSteps persists across DB reads (simulating restart)`() =
        runTest {
    ...
    @Test
    fun `AC-8-3 multiple step batches accumulate correctly`() =
        runTest {
     ...
     */
    @Test
    fun `AC-9-1 route is unaffordable when steps are insufficient`() =
        runTest {
            ...
            assertTrue(
                "Route should not be affordable with 800 steps for 1000-step road",
    ...
    @Test
    fun `AC-9-2 travel deducts step cost from bankedSteps`() =
        runTest {
    ...
    @Test
    fun `AC-9-3 travel blocked when steps insufficient`() =
        runTest {
            stepTrackerService.recordSensorDelta(count = 500, source = StepSource.Simulation)
    ...
    @Test
    fun `AC-10-1 currentTownId updates to destination after travel`() =
        runTest {
    ...
    @Test
    fun `AC-10-2 currentTownId persists to DB (no data loss on restart)`() =
        runTest {
            stepTrackerService.recordSensorDelta(count = 1000, source = StepSource.Simulation)
    ...
    @Test
    fun `AC-10-3 failed travel leaves DB state intact`() =
        runTest {
