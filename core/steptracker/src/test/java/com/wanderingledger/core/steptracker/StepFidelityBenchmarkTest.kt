...
class StepFidelityBenchmarkTest {
    ...
    fun sc002_fidelity_passes_at_all_normal_walking_speeds() {
        ...
        for (speed in speeds) {
            ...
            println("Speed ${speed}m/s → ground=${result.groundTruth}, detected=${result.detected}, F1=${"%.3f".format(result.f1)}")

            assertTrue(
                "F1 at ${speed}m/s (${"%.2f".format(result.f1)}) below threshold $f1Threshold: ground=$groundTruth, detected=${result.detected}",
    ...
    fun sc002_overall_f1_meets_threshold_across_all_speeds() {
        ...
        for (r in summary.results) {
            ...
            println("  [$pass] ${r.label}: GT=${r.groundTruth}, D=${r.detected}, P=${"%.3f".format(r.precision)}, R=${"%.3f".format(r.recall)}, F1=${"%.3f".format(r.f1)}")
        }
        println("  Overall → P=${"%.3f".format(summary.overallPrecision)}, R=${"%.3f".format(summary.overallRecall)}, F1=${"%.3f".format(summary.overallF1)}")
    ...
    fun sc002_detector_sensitivity_bandwidth() {
        for (sensitivity in listOf(0.5f, 0.75f, 1.0f, 1.25f, 1.5f)) {
            ...
            println("sensitivity=$sensitivity → F1=${"%.3f".format(result.f1)}, detected=${result.detected}, GT=${result.groundTruth}")
